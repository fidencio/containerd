...
func (c *CRIImageService) PullImage(ctx context.Context, name string, credentials func(string) (string, string, error), sandboxConfig *runtime.PodSandboxConfig, runtimeHandler string) (_ string, err error) {
	...
	// This can happen when content was garbage collected but image metadata
	// remained, or when switching to a different snapshotter.
	// Stale snapshots are handled by the retry logic in pullWithStaleSnapshotRetry.
	if errdefs.IsFailedPrecondition(err) {
		log.G(ctx).Warnf("Image %q not properly unpacked for snapshotter %q (possible stale metadata), forcing fresh pull; if this persists try: ctr image rm %s", ref, snapshotter, ref)
